...
export class CategoriesPage {
    ...
    async renderCategories() {
        ...
        try {
            ...
            console.log(
                "🚀 ~ CategoriesPage ~ renderCategories ~ categories:",
                categories
            );
            ...
            // Get parent category names for better display
            const categoriesWithParents = await Promise.all(
                categories.map(async (category) => {
                    if (category.parentId) {
                        try {
                            const parent = await getCategoryById(
                                category.parentId
                            );
                            return {
                                ...category,
            ...
            this.populateAddModalParentOptions(
                categoriesWithParents,
                container
            );
            // Add event listeners
    ...
    addEventListeners(container) {
        ...
        container.addEventListener("click", (e) => {
            if (e.target.closest(".edit-btn")) {
                ...
                // Modal will open automatically via data-bs-toggle and data-bs-target
    ...
    async updateCategoryRow(categoryId, updatedData, container) {
        ...
        categoryCell.innerHTML = `
            <div class="d-flex align-items-center">
                ...
                <div>
                    <div class="fw-medium">${
                        updatedData.name || "Unnamed Category"
                    }</div>
                    <small class="text-muted">
                        ${
    ...
    populateAddModalParentOptions(categories, container) {
        ...
        console.log(
            "Populating add modal with root categories:",
            rootCategories
        );
        ...
        if (addParentSelect) {
            ...
            console.log(
                "Add modal parent options populated:",
                addParentSelect.children.length,
                "options"
            );
        } else {
            console.error(
