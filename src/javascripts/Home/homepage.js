...
class HomepageManager {
    ...
    async fetchStats() {
        try {
            // Simple query without where clauses
            const [productsSnapshot, usersSnapshot] = await Promise.all([
                getDocs(query(this.productsCollection, limit(1000))),
                getDocs(query(this.usersCollection, limit(1000))),
            ]);

            // Count available products
            const availableProducts = productsSnapshot.docs.filter((doc) => {
                const data = doc.data();
                return data.status === "available" || !data.status;
            }).length;
            ...
            <div class="col-md-3 col-sm-6 mb-4">
                ...
                : ""
                    ...
                    <div class="card-body d-flex flex-column">
                        <h5 class="product-title mb-2" style="font-size: 1rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${product.name
            }</h5>
                ...
                : ""
                        ...
                        <div class="product-meta mt-auto">
                            <small class="text-muted">Brand: ${product.brand || "SOUKORI"
            }</small><br>
                            <small class="text-muted">Style: ${product.style || "Classic"
            }</small>
                        </div>
                    </div>
                </div>
