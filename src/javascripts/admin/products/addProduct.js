      ...
      <div class="mb-3">
        ...
        <div class="variant-sizes-container">
          <div class="row mb-2 size-row">
            ...
            <div class="col-3">
              <input type="number" step="0.01" class="form-control size-price" placeholder="Price" min="0" required>
            </div>
            <div class="col-2">
    ...
    for (let card of document.querySelectorAll(".variant-card")) {
      ...
      const sizes = Array.from(card.querySelectorAll(".size-row"))
        .map((row) => {
          ...
          return size && quantity > 0 && price > 0
            ? { size, quantity, price }
            : null;
        })
        .filter(Boolean);
      ...
      variants.push({ color, image: imageUrl, sizes });
    }
    return variants;
  }
